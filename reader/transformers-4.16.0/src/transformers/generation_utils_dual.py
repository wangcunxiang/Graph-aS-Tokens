...
class GenerationMixin:
    ...
    def _prepare_model_inputs(
        ...
        if (
            ...
            and self.text_encoder.main_input_name != self.main_input_name
            and self.graph_encoder.main_input_name != self.main_input_name
        ):
            text_input_name = self.text_encoder.main_input_name
            graph_input_name = self.graph_encoder.main_input_name
        else:
            text_input_name = self.main_text_input_name
            graph_input_name = self.main_graph_input_name

        model_kwargs = {k: v for k, v in model_kwargs.items() if v is not None or k != text_input_name or k != graph_input_name}

        # 2. check whether model_input_name is passed as kwarg
        # if yes and `inputs` is None use kwarg inputs
        text_inputs_kwarg = model_kwargs.pop(text_input_name, None)
        if text_inputs_kwarg is not None and inputs is not None:
            raise ValueError(
                f"`inputs`: {inputs}` were passed alongside "
                f"{text_input_name} which is not allowed."
                f"Make sure to either pass {inputs} or {text_input_name}=..."
            )
        elif text_inputs_kwarg is not None:
            text_inputs = text_inputs_kwarg

        graph_inputs_kwarg = model_kwargs.pop(graph_input_name, None)
        if graph_inputs_kwarg is not None and inputs is not None:
            raise ValueError(
                f"`inputs`: {inputs}` were passed alongside "
                f"{graph_input_name} which is not allowed."
                f"Make sure to either pass {inputs} or {graph_input_name}=..."
            )
        elif graph_inputs_kwarg is not None:
            graph_inputs = graph_inputs_kwarg

        # 3. models with `input_ids` can also make use of `inputs_embeds`
        if self._can_retrieve_inputs_from_name(inputs, "text_inputs_embeds", model_kwargs):
            text_inputs, text_input_name = model_kwargs["text_inputs_embeds"], "text_inputs_embeds"

        # 4. Only encoder-decoder models can have non `input_ids` input format
        if not self.config.is_encoder_decoder and text_input_name != "input_ids":
            raise ValueError(
                f"If {text_input_name} is passed as model-specific keyword "
                "input then model has to be an encoder-decoder and not a "
                f"{self.__class__.__name__}."
            )
        if not self.config.is_encoder_decoder and graph_input_name != "input_node_ids":
            raise ValueError(
                f"If {graph_input_name} is passed as model-specific keyword "
                "input then model has to be an encoder-decoder and not a "
                f"{self.__class__.__name__}."
            )
        ...
        if text_inputs is None:
            text_inputs = self._prepare_input_ids_for_generation(bos_token_id, model_kwargs.get("text_encoder_outputs"))
        if graph_inputs is None:
            graph_inputs = self._prepare_input_ids_for_generation(bos_token_id, model_kwargs.get("graph_encoder_outputs"))

        return text_inputs, text_input_name, graph_inputs, graph_input_name, model_kwargs
    ...
    def _prepare_encoder_decoder_kwargs_for_generation(
        self, text_inputs_tensor: torch.Tensor, graph_inputs_tensor: torch.Tensor, model_kwargs,\
            model_text_input_name: Optional[str] = None, model_graph_input_name: Optional[str] = None
    ) -> Dict[str, Any]:
        # 1. get encoder
        ...
        model_text_input_name = model_text_input_name if model_text_input_name is not None else "input_ids"
        model_graph_input_name = model_graph_input_name if model_graph_input_name is not None else "input_node_ids"
        encoder_kwargs["return_dict"] = True
        encoder_kwargs[model_text_input_name] = text_inputs_tensor
        encoder_kwargs[model_graph_input_name] = graph_inputs_tensor
        import copy
        text_encoder_kwargs = {}
        text_encoder_kwargs.update(
            {"input_ids":encoder_kwargs["input_ids"],
             "attention_mask":encoder_kwargs["text_attention_mask"],
        ...
        graph_encoder_kwargs.update(
            {"input_ids":encoder_kwargs["input_node_ids"],
             "input_rel_ids": encoder_kwargs["input_rel_ids"],
             "attention_mask":encoder_kwargs["graph_attention_mask"],
             "output_attentions":encoder_kwargs["output_attentions"],
             "output_hidden_states": encoder_kwargs["output_hidden_states"],
        ...
        model_kwargs["text_encoder_outputs"]: ModelOutput = text_encoder(**text_encoder_kwargs)
        model_kwargs["graph_encoder_outputs"]: ModelOutput = graph_encoder(**graph_encoder_kwargs)
    ...
    def _update_model_kwargs_for_generation(
        ...
        elif "text_past_key_values" in outputs and "graph_past_key_values" in outputs:
            model_kwargs["text_past"] = outputs.text_past_key_values
            model_kwargs["graph_past"] = outputs.graph_past_key_values
        else:
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        self.main_text_input_name = "input_ids"
        self.main_graph_input_name = "input_node_ids"
        text_inputs_tensor, model_text_input_name, graph_inputs_tensor, model_graph_input_name, model_kwargs = self._prepare_model_inputs(inputs, bos_token_id, model_kwargs)
        batch_size = text_inputs_tensor.shape[0]
        ...
        if model_kwargs.get("text_attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            model_kwargs["text_attention_mask"] = self._prepare_attention_mask_for_generation(
                text_inputs_tensor, pad_token_id, eos_token_id
            )
        if model_kwargs.get("graph_attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
            model_kwargs["graph_attention_mask"] = self._prepare_attention_mask_for_generation(
                graph_inputs_tensor, pad_token_id, eos_token_id
            )
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            model_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                text_inputs_tensor, graph_inputs_tensor, model_kwargs, model_text_input_name, model_graph_input_name
            )
        ...
        logits_processor = self._get_logits_processor(
            ...
            encoder_input_ids=text_inputs_tensor,
